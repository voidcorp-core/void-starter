 ...
 * marker, for the reason spelled out in `invite-only.spec.ts`.
 */
...
if (hasDb) {
  test.describe('sign-in / sign-out flow', () => {
    ...
    test.beforeAll(async ({ request }) => {
      // `signUpViaHttp` admits the address first where the project requires it,
      // so this fixture holds on every access mode.
